...
public class Board : MonoBehaviour {
    ...
    public List<Tile> getAlignedTiles() {
        ...
        lines.ForEach(line => {
            if (line.firstTile.Color != Colors.None
                && line.firstTile.Color != Colors.Gray
    ...
    public void initBoard(Level level) {
        ...
        int tileGrowthIncrement = level.enableTileValues ? 1 : 0;
    ...
    public void playLevelCompleteAnimation(Level level) {
        for (int i = 0; i < tiles.Count; i++) {
            Tile currentTile = tiles[i];
            if (level.tiles[i].required
    ...
    private bool doesTileMatch(Tile tile, GameData.Tile tileData) {
        if (tileData.required) {
